...
export const getPreviousCBDate = (date) => {
  try {
    return `${date.getFullYear()}${(date.getMonth()+1).toString().padStart(2, '0')}`;
  } catch (error) {
    console.log("Error trying to create previous CB Date. Creating new date...");
    const tmp = getPreviousMonth(new Date());
    return `${tmp.getFullYear()}${(tmp.getMonth()+1).toString().padStart(2, '0')}`;
  }
}

export const fetchRankingStats = cache(async () => {
  try {
    connectToDB();
  const prevMonth = getPreviousMonth(new Date());
  const cbDate = getPreviousCBDate(prevMonth);
...
export const fetchMember = async (id, returnlean = false) => {
  ...
  export const fetchAllMembers = async (returnlean = false) => {
    try {
      connectToDB();
      if(returnlean) return await Member.find().lean();
