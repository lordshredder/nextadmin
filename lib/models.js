...
const updateDiscordBotWithNewUnit = async (doc) => {
    ...
    try {
        // stringify your data before passing it to request body
...
let bossSchema = new Schema({
    ...
    extraeffect:[{
      effect: {
        type: String,
        default: "TP drain effects are reduced to 0%"}
    }],
    ub:[{
      effect: {
        type: String,
  ...
  bossSchema.post('save', function(doc) {
    updateDiscordBotWithNewBoss(doc);
  });
  
  function updateDiscordBotWithNewBoss(doc){
    console.log("unitSCHEMA different FUNCTION TRIGGERED");
  }
