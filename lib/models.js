...
hitplanSchema.index({ id: 1, team: 1 }, { unique: true });


hitplanSchema.post('save', async function(doc) {
    await updateHitplanOverview(doc);
  });
  
  async function updateHitplanOverview(doc){
    const overview = mongoose.model('hitplanoverviews');
    
    let tmp = await overview.findOne({clan: doc.clan});
    if(!tmp) {
        tmp = new overview({clan: doc.clan});
        await tmp.save();
    }
    await overview.updateHitplanOverview(tmp);


  }

  hitplanSchema.statics.clearCBHitplan = async function(clan = 'all'){
    const overview = mongoose.model('hitplanoverviews');
    try {
        if(clan === 'all'){
            const tmpplan = await this.deleteMany();
            const tmp = await overview.deleteMany();
        } else {
            const tmpplan = await this.deleteOne({clan: clan});
            const tmp = await overview.deleteOne({clan: clan});
        }
    } catch (error) {
        return {error: error}
    }
    return {success: "Cleared Hitplan."}

}


let hitplanOverviewSchema = new Schema({
    clan: {
        type: String,
        require: true,
        unique: true,
...
hitplanOverviewSchema.statics.updateHitplanOverview = async function(overview){
    const hitplans = mongoose.model('hitplans');
    const tmpplan = await hitplans.find({clan: overview.clan}).lean();

    for (let i = 1; i < 6; i++) {
        const namefield = `d${i}`;
        const bossname = `D${i}`;
        overview[namefield].attacks = [];
        overview[namefield].total = 0;
        for (let y = 0; y < tmpplan.length; y++) {
            if(bossname !== tmpplan[y].boss.name) continue;

            overview[namefield].name = bossname;
            overview[namefield].attacks.push({id: tmpplan[y].id, name: tmpplan[y].membername, dmg: tmpplan[y].boss.dmg });
            overview[namefield].total += tmpplan[y].boss.dmg;
        }

        overview[namefield].attacks.sort(function(a, b) {
            return b.dmg - a.dmg; 
          });
    }
    await overview.save();
...
export const HitplanOverview = mongoose.models.hitplanoverviews || mongoose.model("hitplanoverviews", hitplanOverviewSchema);
